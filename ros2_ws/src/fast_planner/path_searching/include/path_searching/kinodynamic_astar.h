...
private:
    ...
    // Combined thrust-vector budget (m/s^2) -- see the search loop's own
    // comment (where `um`, the acceleration primitive itself, is pruned
    // against it) and NonUniformBspline::checkFeasibility's comment for
    // the full derivation. Pruning HERE, not just in the post-optimization
    // feasibility check, stops the search from ever proposing a
    // kinematically-valid-by-max_acc_ primitive that's foredoomed to fail
    // the coupled thrust check downstream.
