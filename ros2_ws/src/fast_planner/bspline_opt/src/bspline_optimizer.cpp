...
void BsplineOptimizer::setParam(std::shared_ptr<FastPlanner> nh) {
  ...
  nh->get_parameter("optimization/thrust_accel_limit", thrust_accel_limit_);
...
void BsplineOptimizer::calcFeasibilityCost(const std::vector<Eigen::Vector3d>& q, double& cost,
  ...
  for (int i = 0; i < q.size() - 2; i++) {
    ...
    // Combined thrust-vector budget: the per-axis check just above treats
    // x/y/z acceleration as three independent budgets, which is exactly
    // the gap the real flight exposed -- a single thrust source has to
    // supply horizontal push AND gravity-compensating lift from the SAME
    // vector, and a trajectory can pass every per-axis check individually
    // while still demanding more combined specific thrust than the
    // airframe has (confirmed via real .ulg telemetry: thrust setpoint
    // pinned at its normalized maximum while still carrying a real 38.9deg
    // tilt at the exact moment altitude control failed). Same quadratic-
    // penalty/analytic-gradient shape as the per-axis term above it, just
    // on the full vector norm instead of each component independently --
    // this is the derivative of (||ai*ts_inv2 + g*z_hat||^2 - L^2)^2 with
    // respect to ai, chained through ai's own dependence on q[i..i+2]
    // exactly like the per-axis term already does.
    Eigen::Vector3d thrust_accel = ai * ts_inv2 + Eigen::Vector3d(0, 0, 9.81);
    double td = thrust_accel.squaredNorm() - thrust_accel_limit_ * thrust_accel_limit_;
    if (td > 0.0) {
      cost += pow(td, 2);

      Eigen::Vector3d temp_t = 4.0 * td * ts_inv2 * thrust_accel;
      gradient[i + 0] += temp_t;
      gradient[i + 1] += -2 * temp_t;
      gradient[i + 2] += temp_t;
    }
  }
}
