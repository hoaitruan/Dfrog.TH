...
private:
  ...
  // Combined thrust-vector budget (m/s^2), independent of limit_acc_ --
  // see setPhysicalLimits' comment for why a per-axis acceleration limit
  // alone cannot express what a single thrust source can actually deliver.
  double limit_thrust_accel_;
...
public:
  ...
  void   setPhysicalLimits(const double& vel, const double& acc, const double& thrust_accel);
