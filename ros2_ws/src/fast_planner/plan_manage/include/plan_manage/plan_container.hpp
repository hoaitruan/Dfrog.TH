...
struct PlanParameters {
  ...
  // Combined thrust-vector budget (m/s^2 of specific thrust, gravity-
  // compensation included) -- see NonUniformBspline::checkFeasibility's
  // own comment for the full derivation and why this is NOT the same
  // constraint as max_acc_. A trajectory can satisfy max_vel_/max_acc_ on
  // every individual axis and still be physically unflyable: a single
  // thrust source has to supply horizontal push AND vertical lift from
  // the same vector, and the per-axis checks never see that coupling.
  double thrust_accel_limit_;
