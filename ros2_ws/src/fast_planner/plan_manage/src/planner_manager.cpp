...
void FastPlannerManager::initPlanModules(std::shared_ptr<FastPlanner> nh) {
  ...
  nh->declare_parameter<double>("manager/thrust_accel_limit", 0.0);
 ...
 // Decalred paramters from Bspilne optimser to avoid again decalration of same paramters
  ...
  nh->declare_parameter<float>("optimization/thrust_accel_limit", 0);
  ...
  // Default -1.0, same sentinel-if-unset convention every other physical
  // limit on this page already uses -- the launch file is expected to
  // always provide a real value at runtime.
  nh->get_parameter_or("manager/thrust_accel_limit", pp_.thrust_accel_limit_, -1.0);
...
bool FastPlannerManager::checkTrajCollision(double& distance) {
  ...
  // rclcpp::Clock().now() -- same fix/rationale as local_data_.start_time_'s
  ...
  while (radius < 6.0 && t_now + fut_t < local_data_.duration_) {
    ...
    // Gate on isObserved(fut_pt): evaluateCoarseEDT -> sdf_map_->getDistance
    // cannot tell "confirmed blocked" from "never observed" (both read
    // esdf_unknown_value_, ~0). Without this gate, dist<0.1 fires on any
    // future trajectory point past the observed frontier -- not
    // hypothetical, this is the exact mechanism behind a 25-warning
    // "current traj in collision" storm (41 replans/5s, 73% search
    // failures) that destabilized real-vehicle tracking and led to the
    // -1.68m ground excursion. Same fix as checkCollisionCallback's
    // isObserved(end_pt_) gate (kino_replan_fsm.cpp:361) and
    // computeFrontier's observed_free check (kino_replan_fsm.cpp:484) --
    // only count a future point as a collision when it's observed AND
    // blocked, never when it's merely not-yet-seen.
    if (edt_environment_->sdf_map_->isObserved(fut_pt) && dist < 0.1) {
      distance = radius;
      return false;
    }
...
bool FastPlannerManager::kinodynamicReplan( Eigen::Vector3d start_pt, Eigen::Vector3d start_vel, Eigen::Vector3d start_acc, Eigen::Vector3d end_pt, Eigen::Vector3d end_vel) {
  ...
  // node_->now() (respects use_sim_time), not an unattached
  // rclcpp::Clock().now() (always wall-clock) -- see node_'s declaration
  // in planner_manager.h for the incident this fixes.
  ...
  pos.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_, pp_.thrust_accel_limit_);
  bool feasible = pos.checkFeasibility(false);

  // Iteration cap raised 3 -> 8, found live: with the thrust budget now
  // enforced at every stage (search pruning, optimizer cost, this
  // feasibility pass), the pipeline still hit the reject-instead-of-fly
  // path 58 times in one verify flight, including cases whose FINAL
  // achieved ratio (tn/to) was as low as ~1.05 -- well under what 3
  // iterations should already cover, which rules out a simple "ratio too
  // big for the iteration budget" explanation on its own.
  // reallocateTime() does one LOCAL sweep per call, patching whichever
  // intervals it finds violating limits in that pass -- a trajectory with
  // several separately-violating intervals (plausible for a path
  // changing both altitude and direction, exactly this project's
  // geometry) can need more independent correction passes than a single
  // global stretch ratio would suggest, regardless of how modest that
  // ratio ends up. More passes gives the sweep more chances to actually
  // clear every violation, not just apply a bigger stretch. The
  // reject-if-still-infeasible check below this loop is the real safety
  // backstop either way -- raising the cap only gives legitimately
  // correctable trajectories more room to actually converge instead of
  // being discarded early.
  int iter_num = 0;
  ...
  // Root cause found offline (test_realloc_jerk.cpp, no flight involved):
  // `feasible`'s FINAL value coming out of the loop above was never
  // checked -- reallocateTime() gets exactly 3 attempts to bring a
  // trajectory within limit_vel_/limit_acc_/limit_thrust_accel_, and if
  // it still hasn't converged after the third, the trajectory was
  // published and flown anyway, with nothing downstream ever told it
  // never actually became feasible. Adding the thrust-budget check gave
  // reallocateTime a THIRD competing constraint to satisfy inside the
  // same 3-iteration budget, making non-convergence more likely to
  // actually matter than it was when only vel/acc were competing for it.
  // Reject here instead, the same way a failed kinodynamic search already
  // does above (both routes lead to the same onReplanFailure() backoff-
  // retry in KinoReplanFSM) -- flying a trajectory that never passed its
  // own feasibility check is not an acceptable substitute for a fresh
  // attempt against updated map/state.
  if (!feasible) {
    std::cout << "[kino replan]: trajectory still infeasible after " << iter_num
              << " reallocateTime iterations -- rejecting instead of flying it." << std::endl;
    return false;
  }
