...
private:
  ...
  void cmdCallback() {
      ...
      if (t_cur < traj_duration_ && t_cur >= 0.0) {
        ...
        // t_cur < 0.0: this tick's clock read is before the trajectory's
        // own declared start_time_. Pre-existing bug in the ported code --
        // pos/vel/acc/yaw/yawdot were left completely uninitialized here
        // and then unconditionally published below, sending garbage
        // (denormalized near-zero floats) on /planning/pos_cmd. Wasn't
        // caught under wall-clock timing (fine-grained, effectively never
        // produces t_cur<0 against a start_time_ set moments earlier);
        // became a real, frequently-hit path once use_sim_time made both
        // nodes' clocks quantized to the sim step (~4ms here), where
        // start_time_ and now() can plausibly read equal-or-inverted
        // across nearby ticks. Skip publishing this tick entirely --
        // same pattern as the receive_traj_ guard above -- rather than
        // publish uninitialized state; the next tick(s) resolve once real
        // time catches up.
